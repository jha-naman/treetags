...
impl Parser {
    ...
    pub fn parse_file(
        ...
        if let Some(tags) =
            self.generate_by_walking(&code, file_path_relative_to_tag_file, extension)
        {
            tags
        } else {
    ...
    /// Generates tags by walking the parsed tree
    pub fn generate_by_walking(
        &mut self,
        code: &[u8],
        file_path_relative_to_tag_file: &str,
        extension: &str,
        ...
        match extension {
            "rs" => self.generate_rust_tags(code, file_path_relative_to_tag_file),
            _ => None,
        }
    }
